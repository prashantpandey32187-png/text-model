...
def generate_3d_model(prompt: str, output_dir: str = "models") -> tuple:
    """
    Generates a 3D model from text prompt using local generation.
    ...
    try:
        ...
        print(f"[INFO] Generating locally (no internet required)...\n")
        
        # Initialize the local generator
        generator = LocalTextTo3D()
        
        # Generate the GLB model locally
        print("[GEN] Generating 3D model from your prompt...")
...
def interactive_mode():
    ...
    while True:
        try:
            # Get user input
            print("-" * 60)
            prompt = input("\n[INPUT] What would you like to create today?\n   > ").strip()
            
            if not prompt:
                print("[WARN] Please enter a prompt!")
                continue
            ...
            print("\n" + "-" * 60)
            another = input("\n[INPUT] Create another model? (y/n): ").strip().lower()
            if another != 'y':
                print("\n[EXIT] Goodbye!")
                break
...
def main():
    ...
    if not args.prompt and not args.interactive:
        try:
            ...
            print("\n\n[WARN] Interactive mode not available in this environment.")
